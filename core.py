...
def render(sds: SDS, intake: dict, shell_path: str, out_path: str, lang: str = "en",
          deepl_api_key: str = None, dcn_override: str = None) -> str:
    ...
    dcn_override: when given (non-empty), used as-is for this language's
    footer DCN instead of the auto-generated one - each language version of
    a document is a distinct controlled copy, so French and Spanish will
    almost always need their own DCN typed in rather than sharing English's.
    """
    L = LANG.get(lang, LANG["en"])
    ...
    dcn = (dcn_override or "").strip() or build_dcn(
        intake.get("sds_number", ""), ver, intake.get("trade_name", ""))
