...
def deepl_translate_batch(texts: list, target_lang: str, api_key: str) -> list:
    ...
    for i in range(0, len(texts), 50):
        chunk = texts[i:i + 50]
        body = json.dumps({"text": chunk, "target_lang": target}).encode("utf-8")
        ...
        out.extend(t["text"] for t in data.get("translations", []))
    return out
...
def build_translation_map(sds: "SDS", intake: dict, lang: str, api_key: str) -> dict:
    ...
    precautionary code line - and translate it in one batched DeepL call.
    ...
    for field in _SECTION1_FREE_FIELDS:
        add(intake.get(field, ""))

    def walk_para_text(p):
        text = p.text
        if translate_code_line(text, lang) is not None:
            return  # handled by the verified phrase table, nothing to translate
        m = _CODE_LINE_RE.match(text)
        # An unrecognized code: translate only the remainder, not the code
        # token itself, so it isn't sent through DeepL and potentially
        # reworded or duplicated in the reconstructed line.
        add(m.group(2) if m else text)
...
def _translate_block(blk, lang, text_map=None):
    """Translate a source-derived block for a non-English output.
    A line starting with a recognized H-code/P-code gets the verified GHS
    phrase, bold code prefix preserved. Everything else is looked up in
    text_map (the DeepL-translated batch built by build_translation_map) -
    if it's not there (no API key configured, or nothing came back for this
    exact string), the block passes through unchanged. Nothing here ever
    invents a translation on its own.
    """
    if lang == "en":
        return blk
    ...
    if translated and translated != lookup_key:
        if m and blk.runs and blk.runs[0].bold and blk.runs[0].text.strip() == m.group(1).strip():
            ...
            code_run_text = blk.runs[0].text
            new_runs = [Run(code_run_text, bold=True), Run(" " + translated)]
        elif m:
            new_runs = [Run(f"{m.group(1)} {translated}")]
        else:
            new_runs = [Run(translated)]
        return Para(new_runs, list_kind=blk.list_kind, level=blk.level, spacing_after=blk.spacing_after)
