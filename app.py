...
def generate_all_languages(sds, intake, shell_path, base_name):
    """Render English, French, and Spanish versions of one document and
    return {suffix: file_path}. Raises DeepLError (caught by callers) if a
    DeepL key is configured but the API call itself fails, so a bad/expired
    key surfaces clearly instead of silently shipping English-only content
