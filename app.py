...
def generate_all_languages(sds, intake, shell_path, base_name, dcn_overrides=None):
    """Render English, French, and Spanish versions of one document and
    return {suffix: file_path}. dcn_overrides is an optional {"EN"/"FR"/"ES":
    dcn_string} dict - each language version is a distinct controlled copy,
    so French and Spanish will almost always need their own typed DCN rather
    than sharing English's auto-generated one. Raises DeepLError (caught by
    callers) if a DeepL key is configured but the API call itself fails, so a
    bad/expired key surfaces clearly instead of silently shipping
    English-only content labeled as translated.
    """
    dcn_overrides = dcn_overrides or {}
    workdir = tempfile.mkdtemp()
    out = {}
    for lang, suffix in (("en", "EN"), ("fr", "FR"), ("es", "ES")):
        fname = f"{base_name}_{suffix}.docx"
        path = os.path.join(workdir, fname)
        render(sds, intake, shell_path, path, lang=lang, deepl_api_key=DEEPL_API_KEY,
              dcn_override=dcn_overrides.get(suffix))
        out[suffix] = path
    return out
...
if mode == "Single file":
    ...
    default_dcn = (build_dcn(intake["sds_number"], intake["version"], intake["trade_name"])
                  if intake["sds_number"] and intake["trade_name"] else "")
    st.markdown("**Footer DCN** (each language is a separate controlled copy - "
               "French and Spanish will almost always need their own, typed in)")
    dcn_cols = st.columns(3)
    dcn_key_suffix = re.sub(r"[^0-9A-Za-z_]", "_",
                            f"{intake['trade_name']}_{intake['sds_number']}_{intake['version']}")
    dcn_en = dcn_cols[0].text_input("DCN - English", default_dcn, key=f"dcn_en_{dcn_key_suffix}")
    dcn_fr = dcn_cols[1].text_input("DCN - French", default_dcn, key=f"dcn_fr_{dcn_key_suffix}")
    dcn_es = dcn_cols[2].text_input("DCN - Spanish", default_dcn, key=f"dcn_es_{dcn_key_suffix}")
    ...
    if st.button("Generate SDS", type="primary", disabled=bool(problems)):
        ...
        try:
            files = generate_all_languages(sds, intake, shell, base,
                                           dcn_overrides={"EN": dcn_en, "FR": dcn_fr, "ES": dcn_es})
...
# =============================================== REBRAND ACROSS VERTICALS ===
    ...
    for v in chosen:
        with st.expander(f"{v}", expanded=True):
            ...
            dc = st.columns(3)
            dcn_key_suffix = re.sub(r"[^0-9A-Za-z_]", "_",
                                    f"{v}_{intake['trade_name']}_{intake['sds_number']}_{intake['version']}")
            dcn_jobs[v] = {
                "EN": dc[0].text_input("DCN - English", default_dcn, key=f"dcn_en_{dcn_key_suffix}"),
                "FR": dc[1].text_input("DCN - French", default_dcn, key=f"dcn_fr_{dcn_key_suffix}"),
                "ES": dc[2].text_input("DCN - Spanish", default_dcn, key=f"dcn_es_{dcn_key_suffix}"),
            }
        ...
        try:
            ...
            for v, intake in jobs.items():
                ...
                files = generate_all_languages(sds_copy, intake,
                                               ...
                                               base_name, dcn_overrides=dcn_jobs[v])
...
# ============================================================ BULK REFORMAT ===
    ...
    for f in uploads:
        ...
        with st.expander(f.name, expanded=False):
            ...
            dc = st.columns(3)
            dcn_key_suffix = re.sub(r"[^0-9A-Za-z_]", "_",
                                    f"{f.name}_{intake['trade_name']}_{intake['sds_number']}_{intake['version']}")
            dcn_jobs[f.name] = {
                "EN": dc[0].text_input("DCN - English", default_dcn, key=f"dcn_en_{dcn_key_suffix}"),
                "FR": dc[1].text_input("DCN - French", default_dcn, key=f"dcn_fr_{dcn_key_suffix}"),
                "ES": dc[2].text_input("DCN - Spanish", default_dcn, key=f"dcn_es_{dcn_key_suffix}"),
        ...
        try:
            ...
            for fname_in, intake in jobs.items():
                ...
                files = generate_all_languages(s, intake, shell, out_base,
                                               dcn_overrides=dcn_jobs[fname_in])
                all_files.update(files)
                names.extend(os.path.basename(p) for p in files.values())
        except DeepLError as e:
            st.error(f"Translation failed: {e}")
