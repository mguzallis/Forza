...
from core import (VERTICALS, extract, apply_intake, render, preflight,
                  build_dcn, build_sds_field, DeepLError)
...
def sds_number_fields(key_suffix, sds_number, version, trade_name):
    """Three per-language SDS # inputs, each prefilled with the same detected
    SDS #, plus the DCN that would be derived from it. Returns {"EN"/"FR"/
    "ES": sds_number_string}. Each language is a distinct controlled copy -
    French and Spanish will almost always need their own SDS # typed in; the
    footer DCN is derived from whichever SDS # applies, not typed separately.
    """
    st.markdown("**SDS # per language** (each is a separate controlled copy - "
               "the footer DCN is derived from this, and French/Spanish will "
               "almost always need their own)")
    safe_key = re.sub(r"[^0-9A-Za-z_]", "_", f"{key_suffix}_{trade_name}_{sds_number}_{version}")
    cols = st.columns(3)
    out = {}
    for col, label, suffix in zip(cols, ("English", "French", "Spanish"), ("EN", "FR", "ES")):
        val = col.text_input(f"SDS # - {label}", sds_number, key=f"sdsnum_{suffix}_{safe_key}")
        out[suffix] = val
        dcn_preview = (build_dcn(val, version, trade_name) if val and trade_name else "-")
        col.caption(f"DCN: {dcn_preview}")
    return out


def generate_all_languages(sds, intake, shell_path, base_name, sds_number_overrides=None):
    """Render English, French, and Spanish versions of one document and
    return {suffix: file_path}. sds_number_overrides is an optional
    {"EN"/"FR"/"ES": sds_number_string} dict - each language version is a
    distinct controlled copy, so French and Spanish will almost always need
    their own SDS # rather than sharing English's; the footer DCN is derived
    from whichever SDS # applies to that language, not typed separately.
    Raises DeepLError (caught by callers) if a DeepL key is configured but
    the API call itself fails, so a bad/expired key surfaces clearly instead
    of silently shipping English-only content labeled as translated.
    """
    sds_number_overrides = sds_number_overrides or {}
    workdir = tempfile.mkdtemp()
    out = {}
    for lang, suffix in (("en", "EN"), ("fr", "FR"), ("es", "ES")):
        lang_intake = dict(intake)
        override = sds_number_overrides.get(suffix)
        if override:
            lang_intake["sds_number"] = override
        fname = f"{base_name}_{suffix}.docx"
        path = os.path.join(workdir, fname)
        render(sds, lang_intake, shell_path, path, lang=lang, deepl_api_key=DEEPL_API_KEY)
        out[suffix] = path
    return out
...
if mode == "Single file":
    ...
    st.divider()
    st.metric("SDS # field", build_sds_field(intake["sds_number"], intake["version"])
    ...
    sds_numbers = sds_number_fields("single", intake["sds_number"], intake["version"],
                                    intake["trade_name"])
    ...
    if st.button("Generate SDS", type="primary", disabled=bool(problems)):
        ...
        try:
            files = generate_all_languages(sds, intake, shell, base,
                                           sds_number_overrides=sds_numbers)
            ...
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zip_out:
                for suffix, path in files.items():
                    zip_out.write(path, os.path.basename(path))
            ...
            if DEEPL_API_KEY:
                st.caption("Hard page breaks are intentionally not carried over. "
                          "French/Spanish content is machine-translated - review before "
                          "distribution.")
            else:
                st.caption("Hard page breaks are intentionally not carried over. No DeepL "
                          "API key is configured, so French/Spanish only have labels and "
                          "verified hazard-code phrases translated; free-text content stays "
                          "in English until a key is added.")
    ...
    for v in chosen:
        with st.expander(f"{v}", expanded=True):
            ...
            if p:
                ...
                for msg in p:
                    st.warning(msg)
            sds_num_jobs[v] = sds_number_fields(f"rebrand_{v}", intake["sds_number"],
                                                intake["version"], intake["trade_name"])
    ...
    if st.button("Generate all verticals", type="primary", disabled=bool(problems)):
        try:
            ...
            for v, intake in jobs.items():
                ...
                files = generate_all_languages(sds_copy, intake,
                                               os.path.join(SHELL_DIR, f"SDS_Shell_{v}.docx"),
                                               base_name, sds_number_overrides=sds_num_jobs[v])
                all_files.update(files)
                names.extend(os.path.basename(p) for p in files.values())
        except DeepLError as e:
            st.error(f"Translation failed: {e}")
        else:
            buf = io.BytesIO()
...
# ============================================================ BULK REFORMAT ===
    ...
    parsed, jobs, problems, sds_num_jobs = {}, {}, {}, {}
    for f in uploads:
        ...
        with st.expander(f.name, expanded=False):
            ...
            if p:
                ...
                for msg in p:
                    st.warning(msg)
            sds_num_jobs[f.name] = sds_number_fields(f"bulk_{f.name}", intake["sds_number"],
                                                     intake["version"], intake["trade_name"])
    ...
    if st.button("Generate all files", type="primary", disabled=bool(problems)):
        try:
            ...
            for fname_in, intake in jobs.items():
                ...
                files = generate_all_languages(s, intake, shell, out_base,
                                               sds_number_overrides=sds_num_jobs[fname_in])
                all_files.update(files)
                names.extend(os.path.basename(p) for p in files.values())
        except DeepLError as e:
            st.error(f"Translation failed: {e}")
